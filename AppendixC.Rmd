...
writeLines(
  c(
    "```{r}                     ---> abertura do chunck",
    "#Aqui vai seu código R     ---> código   ",
    "```                        ---> fechamento do chunck"
  )
)
```
...
```{r letras-ufsm, eval = TRUE, echo = FALSE, message = TRUE, results = 'hold', comment = ""}
writeLines(c(
...
```{r chunck4, eval = TRUE, echo = FALSE}
# Carregando a pacote 'ggplot2' para gráficos elegante
library(ggplot2)
# Pacote com dados de temperatura média global
library(gcookbook)
# Primeiras linhas dos dados 'climate'
head(climate)
# gráfico da série de temperatura global
qplot(x = Year, y = Anomaly1y, data = climate) + geom_smooth()
# mesmo gráfico com a função plot() da base do R
...
```{r chunck5, echo=FALSE, eval = TRUE}
writeLines(
c(
"```{r}"
...
```{r chunck6, echo = FALSE, eval = TRUE}
# pacote com a função kable para saída de tabelas
library(knitr)
# pacote com dados de temperatura global
library(gcookbook)
# Aplicando a função 'summary()' ao dataframe airquality.
resumo <- summary(airquality)
# Convertendo o formato da saída das tabelas
kable(resumo, caption = "Tabela 1: Resumo estatístico dos dados de 'qualidade do ar' de NY")
```
...
```{r chunck8, echo = FALSE, comment=" ", eval = TRUE}
writeLines(
c(
...
```{r chunck9, echo = FALSE, eval = TRUE}
writeLines(
c(
