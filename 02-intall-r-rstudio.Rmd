...
A interação do usuário com o `r rblue` é por meio da linha de comando. Essa interação pode ser facilitada com o uso do [RStudio](http://www.rstudio.com/).

A seguir descreve-se como instalar o `r rblue` no Windows e no Linux Ubuntu. A forma de instalação do `r rblue` no Linux tenta ser mais didática do que prática. Alguns comandos linux básicos serão utilizados, mas mesmo quem não é usuário linux será capaz de entendê-los.

## Instalando o `r rblue`

O `r rblue` pode ser instalado a partir dos [binários pré-compilados](https://cran.r-project.org/bin/) ou do [código fonte](https://cran.r-project.org/sources.html). Aqui, descreve-se a instalação do `r rblue` a partir dos binários
...
A instalação do `r rblue` para Windows a partir do executável acima incluirá na instalação uma GUI chamada `RGui.exe`, mostrada abaixo.
...
Entretanto, os pacotes do `r rblue` recém lançados são compilados para última versão do R. Então você pode ter restrições de uso de pacotes novos, os quais geralmente incluem o estado da arte de análise de dados.
...
Esse caminho fica então visível ao `r rblue`, o que pode ser verificado executando a função `.libPaths()` no console do `r rblue`. Abra o R:
...
O RStudio é um ambiente integrado de desenvolvimento ([IDE](http://en.wikipedia.org/wiki/Integrated_development_environment)) construído especificamente para o `r rblue`. O RStudio para Desktop pode ser baixado gratuitamente e é multiplataforma.
...
```{r rstudio-choose, echo = FALSE,  fig.align='center', fig.width=12}
knitr::include_graphics('images/rstudio-choose.png')
```

  5. Selecione sua plataforma
  
```{r, rstudio-plat, echo = FALSE, fig.align='center', fig.width=12}
