...
Essa é justamente a idéia do operador *pipe* `%>%`: passar a saída de uma função para outra função como a entrada dessa função por meio de uma seqüência de etapas. O operador `%>%` está disponível no `r rblue` através do pacote [magrittr](https://cran.r-project.org/web/packages/magrittr/vignettes/magrittr.html).
...
```{block, pipe-linux, type='rmdtip'}
No exemplo anterior nós introduzimos a função `dplyr::nth()`. Ela é equivalente ao operador conchetes `[` da base do R. Se `a <- 5:1` então as instruções abaixo produzem resultados equivalentes:
