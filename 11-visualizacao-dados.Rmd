...
O `r rblue` possui uma poderosa [plataforma de ferramentas gráficas](http://cran.r-project.org/web/views/Graphics.html) para análise e visualização de dados. Os sistemas gráficos são: 
...
Nesta aula será dado foco a produção de gráficos usando o **sistema *base*** do `r rblue`.
...
A função `plot()` é o carro chefe do sistema base do `r rblue`. Quando especificamos apenas uma variável de entrada é plotado a variável no eixo y em função de seu tamanho `1:length(x)` no eixo x.
