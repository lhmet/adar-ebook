...
Você pode usar as teclas `r emo::ji("arrow_up")` e `r emo::ji("arrow_down")` para navegar entre as expressões já avaliadas pelo <code class="sourceCode bash"><span class="ex">R</span></code>. O que é útil quando precisamos repetir um comando anterior com alguma mudança ou para corrigir um erro de digitação ou a omissão de um parentêses.
