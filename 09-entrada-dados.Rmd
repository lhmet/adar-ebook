...
O `r rblue` é capaz de importar dados de uma diversidade de fontes, formatos e 
...
`r rblue` (`readRDS()`, `load()`) e funções de pacotes específicos para importar
...
Para carregar diversos pacotes de uma vez só, você pode usar a função 
...
O `r rblue` possui uma variedade de funções para se obter informações do 
...
`r rblue`, ela é vinculada a um diretório de trabalho 
(*working directory*, `wd`). A função `getwd()` retorna o diretório de trabalho
da sua sessão do `r rblue`.
...
É neste local onde o `r rblue` e o RStudio irão salvar gráficos, documentos, 
...
Você pode configurar o diretório de trabalho pelo menu do Rstudio 
...
facilmente importados no `r rblue`. 
...
disponíveis no `r rblue` (veja o [Manual de Importação e Exportação do R](https://cran.r-project.org/doc/manuals/r-release/R-data.htmlpara ter noção)).
...
knitr::kable(tab_rio,
  ...
  booktabs = TRUE,
  caption = 
    "Versão resumida da tabela com os formatos suportados pelo pacote **rio**.",
  align = "c"
)
```
...
acima foi usada a função `read.fwf()` do `r rblue` para ler os dados. Os 
...
`r rblue` independente do sistema operacional e da arquitetura do computador, 
...
`r rblue` usando as funções da base do R, consulte o manual [R Data Import/Export](http://cran.r-project.org/doc/manuals/r-release/R-data.html) e
