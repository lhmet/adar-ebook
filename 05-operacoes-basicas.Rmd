...
esp_num_df <- tibble::tibble(`operação` = oper, resultado = res)
...
Outra constante especial do `r rblue` é o `NA` (*Not Available*) que representa **valor faltante**, um problema inerente a maioria dos conjuntos de dados ambientais. Qualquer operação envolvendo `NA` resultará em `NA` (Tabela \@ref(tab:tab-nas)). 
...
É preciso ter cuidado ao nomear variáveis no  `r rblue` porque existem algumas regras:
